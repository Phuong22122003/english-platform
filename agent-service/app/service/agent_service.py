...
class AgentService:
    ...
    async def get_image(self, name, description=None, max_size=(300, 300)):
        ...
        if files:
            ...
            with Image.open(old_path) as img:
                img.thumbnail(max_size)  # resize giữ tỉ lệ, max width/height = 300
                img.save(new_path)
    ...
    async def create_topic(self, topic_type:str, description: str):   
        wb = Workbook()
        ws = wb.active
        ...
        if topic_type.upper() == 'VOCABULARY':
            ...
            print(settings.CONTENT_SERVICE_URL + f"/vocabulary/topics/file")
            print(response.text)
            for _, f in files:
                content = f[1]
